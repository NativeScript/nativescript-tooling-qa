...
def sync_hello_world_js(app_name,
                        platform,
                        device,
                        bundle=True,
                        hmr=True,
    ...
    __sync_hello_world_js_ts(
        ...
        default_andr_sdk=default_andr_sdk)


def sync_hello_world_ts(app_name,
                        platform,
                        device,
                        bundle=True,
                        hmr=True,
...
def run_hello_world_js_ts(app_name,
    ...
    TnsLogs.wait_for_log(
        log_file=result.log_file, string_list=strings, timeout=timeout)
    ...
    if platform == Platform.ANDROID:
        ...
        TnsAssert.string_in_android_manifest(
            apk_path, 'compileSdkVersion="{0}"'.format(default_andr_sdk))
    if snapshot and Settings.HOST_OS != OSType.WINDOWS:
        TnsAssert.snapshot_build(
            TnsPaths.get_apk_path(app_name=app_name, release=True),
...
def __sync_hello_world_js_ts(app_type,
    ...
    device.wait_for_color(
        color=Colors.LIGHT_BLUE, pixel_count=blue_count * 2, delta=25)
...
def preview_hello_world_js_ts(app_name,
    ...
    device.wait_for_text(
        text=Changes.JSHelloWord.JS.old_text, timeout=90, retry_delay=5)
...
def preview_sync_hello_world_js_ts(app_type,
                                   ...
                                   instrumented=False,
                                   click_open_alert=False):
    result = preview_hello_world_js_ts(
        app_name=app_name,
        platform=platform,
        device=device,
        bundle=bundle,
    ...
    if hmr and instrumented and Settings.HOST_OS != OSType.WINDOWS:
        ...
        TnsLogs.wait_for_log(
            log_file=result.log_file, string_list=strings, timeout=90)
    device.wait_for_color(
        color=Colors.LIGHT_BLUE, pixel_count=blue_count * 2, delta=25)
