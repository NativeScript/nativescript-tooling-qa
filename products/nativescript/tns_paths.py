...
class TnsPaths(object):
    ...
    def get_app_node_modules_path(app_name, path=Settings.TEST_RUN_HOME):
        return os.path.join(
            TnsPaths.get_app_path(app_name=app_name, path=path),
            'node_modules')

    @staticmethod
    def get_path_app_resources(app_name, path=Settings.TEST_RUN_HOME):
        return os.path.join(
            TnsPaths.get_app_path(app_name=app_name, path=path), 'app',
            'App_Resources')

    @staticmethod
    def get_path_app_resources_main_android(app_name,
    ...
    def get_platforms_android_folder(app_name, path=Settings.TEST_RUN_HOME):
        return os.path.join(
            TnsPaths.get_app_path(app_name=app_name, path=path), 'platforms',
            'android')
    ...
    @staticmethod
    def get_platforms_android_src_main_path(app_name,
                                            path=Settings.TEST_RUN_HOME):
        return os.path.join(
            TnsPaths.get_platforms_android_folder(
                app_name=app_name, path=path), 'app', 'src', 'main')

    @staticmethod
    def get_platforms_android_app_path(app_name, path=Settings.TEST_RUN_HOME):
        return os.path.join(
            TnsPaths.get_platforms_android_folder(
                app_name=app_name, path=path), 'app', 'src', 'main', 'assets',
            'app')

    @staticmethod
    def get_platforms_ios_app_path(app_name, path=Settings.TEST_RUN_HOME):
        return os.path.join(
            TnsPaths.get_platforms_ios_folder(app_name=app_name, path=path),
            app_name, 'app')

    @staticmethod
    def get_platforms_android_npm_modules(app_name,
                                          path=Settings.TEST_RUN_HOME):
        return os.path.join(
            TnsPaths.get_platforms_android_folder(
                app_name=app_name, path=path), 'app', 'src', 'main', 'assets',
    ...
    def get_platforms_ios_npm_modules(app_name, path=Settings.TEST_RUN_HOME):
        return os.path.join(
            TnsPaths.get_platforms_ios_folder(app_name=app_name, path=path),
            app_name, 'app', 'tns_modules')

    @staticmethod
    def get_apk_path(app_name, release=False, path=Settings.TEST_RUN_HOME):
        build_path = os.path.join(
            TnsPaths.get_platforms_android_folder(
                app_name=app_name, path=path), 'app', 'build')
        if release:
            return os.path.join(build_path, 'outputs', 'apk', 'release',
                                'app-release.apk')
        else:
            return os.path.join(build_path, 'outputs', 'apk', 'debug',
                                'app-debug.apk')
    ...
    def get_ipa_path(app_name,
        ...
        if for_device:
            if release:
                return os.path.join(base_path, 'Release-iphoneos',
                                    '{0}.ipa'.format(native_app_name))
            else:
                return os.path.join(base_path, 'Debug-iphoneos',
                                    '{0}.ipa'.format(native_app_name))
        else:
            if release:
                return os.path.join(base_path, 'Release-iphonesimulator',
                                    '{0}.app'.format(native_app_name))
            else:
                return os.path.join(base_path, 'Debug-iphonesimulator',
                                    '{0}.app'.format(native_app_name))
    ...
    def get_bundle_id(app_name):
        return 'org.nativescript.' + TnsPaths.__get_trimmed_app_name(
            app_name=app_name)
