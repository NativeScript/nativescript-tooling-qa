...
class Tns(object):
    ...
    def plugin_create(plugin_name, path=None, log_trace=False, verify=True):
        ...
        result = Tns.exec_command(command="plugin create --includeTypeScriptDemo=y " + plugin_name, path=path,
                                  log_trace=log_trace)
        ...
        if verify:
            ...
            assert "Solution for {0}".format(plugin_name) + " was successfully created" in result.output, 'Missing message in output.'
