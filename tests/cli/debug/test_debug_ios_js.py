...
class DebugIOSJSTests(TnsRunIOSTest):
    ...
    def test_001_debug_elements(self):
        ...
        Tns.debug(app_name=self.app_name, platform=Platform.IOS)
        ...
        self.sim.wait_for_text(text=change.new_text)
    ...
    def test_010_debug_console_log(self):
        ...
        # Run `tns debug` wait until debug url is in the console
        result = Tns.debug(app_name=self.app_name, platform=Platform.IOS)
        ...
        assert self.sim.wait_for_log(text='Test Debug!'), 'Console logs not avaiable in device logs.'
    ...
    def test_020_debug_sources(self):
        ...
        # Tap on TAP button in simulator and check it is hit
        self.sim.click(text="TAP", case_sensitive=True)
