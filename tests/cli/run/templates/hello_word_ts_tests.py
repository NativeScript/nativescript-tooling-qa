...
class RunAndroidTSTests(TnsRunTSTests):
    def test_100_run_android(self):
        SyncHelpers.sync_hello_world_ts(app_name=self.app_name, platform=Platform.ANDROID, device=self.emu)

    def test_200_run_android_bundle(self):
        SyncHelpers.sync_hello_world_ts(app_name=self.app_name, platform=Platform.ANDROID, device=self.emu, bundle=True)
...
class RunIOSTSTests(TnsRunTSTests):
    def test_100_run_ios(self):
        SyncHelpers.sync_hello_world_ts(app_name=self.app_name, platform=Platform.IOS, device=self.sim)

    def test_200_run_ios_bundle(self):
        SyncHelpers.sync_hello_world_ts(app_name=self.app_name, platform=Platform.IOS, device=self.sim, bundle=True)
