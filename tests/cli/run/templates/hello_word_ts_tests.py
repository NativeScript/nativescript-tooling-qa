...
class RunAndroidTSTests(TnsRunTSTests):
    def test_100_run_android(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.ANDROID, self.emu)

    def test_200_run_android_bundle(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.ANDROID, self.emu, bundle=True)

    def test_210_run_android_bundle_hmr(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.ANDROID, self.emu, bundle=True, hmr=True)

    def test_300_run_android_bundle_aot(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.ANDROID, self.emu, bundle=True, aot=True)

    def test_310_run_android_bundle_uglify(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.ANDROID, self.emu, bundle=True, uglify=True)

    def test_320_run_android_bundle_aot_and_uglify(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.ANDROID, self.emu, bundle=True, aot=True, uglify=True)
...
class RunIOSTSTests(TnsRunTSTests):
    def test_100_run_ios(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.IOS, self.sim)

    def test_200_run_ios_bundle(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.IOS, self.sim, bundle=True)

    def test_210_run_ios_bundle_hmr(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.IOS, self.sim, bundle=True, hmr=True)

    def test_300_run_ios_bundle_aot(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.IOS, self.sim, bundle=True, aot=True)

    def test_310_run_ios_bundle_uglify(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.IOS, self.sim, bundle=True, uglify=True)

    def test_320_run_ios_bundle_aot_and_uglify(self):
        SyncHelpers.sync_hello_world_ts(self.app_name, Platform.IOS, self.sim, bundle=True, aot=True, uglify=True)
