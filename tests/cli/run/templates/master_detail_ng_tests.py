...
class TnsRunMasterDetailTests(TnsRunTest):
    ...
    def test_200_run_android_no_bundle_no_hmr(self):
        sync_master_detail_ng(self.app_name, Platform.ANDROID, self.emu, bundle=False, hmr=False)

    @unittest.skipIf(Settings.HOST_OS != OSType.OSX, 'iOS tests can be executed only on macOS.')
    def test_200_run_ios_no_bundle_no_hmr(self):
        sync_master_detail_ng(self.app_name, Platform.IOS, self.sim, bundle=False, hmr=False)
