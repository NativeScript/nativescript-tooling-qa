...
class AndroidAppBundleTests(TnsRunTest):
    ...
    def bundletool_build(bundletool_path, path_to_aab, path_to_apks):
        build_command = ('java -jar {0} build-apks --bundle="{1}" --output="{2}" --ks="{3}" --ks-pass=pass:"{4}" --ks-key-alias="{5}"' \
                          ' --key-pass=pass:"{6}"').format(bundletool_path, path_to_aab, path_to_apks, Android.ANDROID_KEYSTORE_PATH,
                                                           Android.ANDROID_KEYSTORE_PASS, Android.ANDROID_KEYSTORE_ALIAS, Android.ANDROID_KEYSTORE_ALIAS_PASS)
        result = run(build_command)
        assert not "Error" in result.output, "create of .apks file failed"

    @staticmethod
    def bundletool_deploy(bundletool_path, path_to_apks, device_id):
        deploy_command = ('java -jar {0} install-apks --apks="{1}" --device-id={2}').format(bundletool_path, path_to_apks, device_id)
        result = run(deploy_command)
        assert not "Error" in result.output, "deploy of app failed"
    ...
    def test_200_build_android_app_bundle(self):
        ...
        build_command = 'build android --path {0} --aab'.format(self.app_path)
        result = Tns.build_android(self.app_path, aab=True, verify=False)
        # There is an issue at the moment that the path is not shown in log. TODO: uncomment this when the issue is fixed
    ...
    def test_205_build_android_app_bundle_env_snapshot(self):
        ...
        result = Tns.build_android(self.app_path, aab=True, release=True, snapshot=True, uglify=True, verify=False)
        # There is an issue at the moment that the path is not shown in log. TODO: uncomment this when the issue is fixed
        # assert "The build result is located at:" in result.output
        # assert path_to_aab in result.output
        assert File.exists(path_to_aab)
