        ...
        if os.name == 'nt':
            if "tns-dist" in package:
                package = package.replace(
                    "/tns-dist/",
                    "\\\\telerik.com\\distributions\\DailyBuilds\\NativeScript\\"
                )
                package = package.replace("/", "\\")
                ...
                if name == 'tns-core-modules' and "PR" in package and ".tgz" in package:
                    if "release" in os.environ.get("ghprbTargetBranch",
                                                   "master"):
                        return name + '@rc'
                    else:
                        return name + '@next'
...
class Packages(object):
    ...
    NS_SCHEMATICS = resolve_package(
        name='@nativescript/schematics', variable='nativescript_schematics')
    NG_CLI = resolve_package(
        name='@angular/cli', variable='ng_cli', default='latest')
    ...
    PREVIEW_PATH = os.environ.get(
        'preview_folder_path',
        os.path.join("/tns-dist", "Playground", "ns-play-dev",
                     __default_preview_folder))
    PLAYGROUND_PATH = os.environ.get(
        'playground_folder_path',
        os.path.join("/tns-dist", "Playground", "ns-play",
                     __default_preview_folder))

    PREVIEW_APP_IOS = os.path.join(PREVIEW_PATH, "nsplaydev.tgz")
    PREVIEW_APP_ANDROID = os.path.join(PREVIEW_PATH,
    ...
    MODULES = resolve_package(
        name='tns-core-modules', variable='tns_core_modules')
    ANGULAR = resolve_package(
        name='nativescript-angular', variable='nativescript_angular')
    WEBPACK = resolve_package(
        name='nativescript-dev-webpack', variable='nativescript_dev_webpack')
    TYPESCRIPT = resolve_package(
        name='nativescript-dev-typescript',
        variable='nativescript_dev_typescript')
    SASS = resolve_package(
        name='nativescript-dev-sass', variable='nativescript_dev_sass')
...
class Emulators(object):
    EMU_API_19 = EmulatorInfo(
        avd=os.environ.get('EMU_API_19', 'Emulator-Api19-Default'),
        os_version=4.4,
        port='5560',
        emu_id='emulator-5560')
    EMU_API_23 = EmulatorInfo(
        avd=os.environ.get('EMU_API_23', 'Emulator-Api23-Default'),
        os_version=6.0,
        port='5562',
        emu_id='emulator-5562')
    EMU_API_24 = EmulatorInfo(
        avd=os.environ.get('EMU_API_24', 'Emulator-Api24-Default'),
        os_version=7.0,
        port='5568',
        emu_id='emulator-5568')
    EMU_API_26 = EmulatorInfo(
        avd=os.environ.get('EMU_API_26', 'Emulator-Api26-Google'),
        os_version=8.0,
        port='5564',
        emu_id='emulator-5564')
    EMU_API_28 = EmulatorInfo(
        avd=os.environ.get('EMU_API_28', 'Emulator-Api28-Google'),
        os_version=9.0,
        port='5566',
        emu_id='emulator-5566')
    EMU_API_29 = EmulatorInfo(
        avd=os.environ.get('EMU_API_29', 'Emulator-Api29-Google'),
        os_version=10.0,
        port='5568',
        emu_id='emulator-5568')
    EMU_API_23_64 = EmulatorInfo(
        avd=os.environ.get('EMU_API_23_64', 'Emulator-Api23-64'),
        os_version=6.0,
        port='5570',
...
Emulators.DEFAULT = resolve_default_emulator("EMULATOR_API_VERSION",
                                             Emulators.EMU_API_23)


class Simulators(object):
    SIM_IOS10 = SimulatorInfo(
        name=os.environ.get('SIM_IOS10', 'iPhone7_10'),
        device_type='iPhone 7',
        sdk=10.0)
    SIM_IOS11 = SimulatorInfo(
        name=os.environ.get('SIM_IOS11', 'iPhone7_11'),
        device_type='iPhone 7',
        sdk=11.2)
    SIM_IOS12 = SimulatorInfo(
        name=os.environ.get('SIM_IOS12', 'iPhoneXR_12'),
        device_type='iPhone XR',
        sdk=12.0)
    SIM_IOS13 = SimulatorInfo(
        name=os.environ.get('SIM_IOS13', 'iPhoneXR_13'),
        device_type='iPhone XR',
        sdk=13.1)
    DEFAULT = SIM_IOS12
